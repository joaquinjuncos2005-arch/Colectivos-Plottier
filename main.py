...
CONSULTAS = [
    ...
    # Se muestran en pantalla y también se usan como puntos GPS.
    # =========================================================
    {"seccion": "MIS FAVORITOS", "parada": "1260", "calle": "Av. Belgrano y Pulmari", "lat": -38.9533476, "lon": -68.2375682, "linea": "50B", "cod": "1014", "mostrar": True, "mapa": True},
    {"seccion": "MIS FAVORITOS", "parada": "NV1311", "calle": "Santa Cruz y Riavitz", "lat": -38.957381, "lon": -68.226198, "linea": "50B", "cod": "1014", "mostrar": True, "mapa": True},
    {"seccion": "MIS FAVORITOS", "parada": "NV2316", "calle": "Martellota y Las Lajas", "lat": -38.9508355, "lon": -68.228275, "linea": "50B", "cod": "1014", "mostrar": True, "mapa": True},

    # =========================================================
    # MISMAS FUENTES GPS QUE app.py (las más fiables)
    # =========================================================
    {"seccion": "CABECERA", "parada": "NV2000", "calle": "Cabecera Plottier", "linea": "50B", "cod": "1014", "mostrar": True, "mapa": False, "cabecera": True},
    {"seccion": "CABECERA", "parada": "NV1014", "calle": "Cabecera Neuquén", "lat": -38.946678, "lon": -68.0576064, "linea": "50A", "cod": "1013", "mostrar": True, "mapa": False, "cabecera": True},
    {"seccion": "BARRIDO", "parada": "NV1058", "calle": "", "linea": "50B", "cod": "1014", "mostrar": False, "mapa": False},
]
...
<script>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
    ...
    .stop-address { font-size: 11px; color: #A6ADC8; margin: -4px 0 8px; }
    .stop-marker {
      background: #F5E0DC;
...
<body>
  <header>
    ...
    <p class="sub">GPS y recorridos oficiales en vivo · 50A / 50B</p>
  </header>
  ...
  <div class="credits">
    Desarrollado por <span class="author">juncos</span>
    <span class="alias-badge">JJ</span>
  </div>
  ...
  <script>
    ...
    function dibujarParadas(paradas) {
      ...
      paradas.forEach(p => {
        if (!p || !p.mapa || p.lat == null || p.lon == null) return;

        const id = `${p.parada}-${p.linea}`;
        idsRecibidos.add(id);
        ...
        const titulo = p.calle ? p.calle : `Parada ${p.parada}`;
        const popup = `
          <div style="font-family:sans-serif;font-size:13px;line-height:1.4;min-width:170px;">
            <strong style="color:#111;font-size:14px;">Parada ${p.parada}</strong><br>
    ...
    async function pedirDatos() {
      ...
      try {
        const res = await fetch('/api/arribos', { cache: 'no-store' });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        
        renderTarjetas(data.items);
        dibujarParadas(data.items);
      ...
      items.forEach(item => {
        ...
        html += `<div class="card">
          <div class="card-header">
            ...
            <span class="stop-tag">${item.cabecera ? 'Cabecera' : 'Parada'} ${item.parada}</span>
          </div>
          ${item.calle ? `<div class="stop-address">📍 ${item.calle}</div>` : ''}`;

        if (!item.arribos || item.arribos.length === 0) {
          html += `<div class="empty">${item.cabecera ? 'Sin unidades reportando hacia cabecera' : 'Sin unidades reportando en esta parada'}</div>`;
        } else {
          item.arribos.forEach(c => {
            const botonVer = (c.lat != null && c.lon != null && c.lat !== '' && c.lon !== '')
    ...
    for item in CONSULTAS:
        try:
            ...
            for c in arribos_raw:
                ...
                try:
                    f_lat = float(str(lat).replace(',', '.')) if lat is not None and str(lat).strip() else None
                    f_lon = float(str(lon).replace(',', '.')) if lon is not None and str(lon).strip() else None
                    if f_lat is not None and f_lon is not None and -40 < f_lat < -37 and -70 < f_lon < -66:
                        vincular_o_crear_colectivo(
                            item["linea"], ramal, sentido, sentido_code,
                            tiempo, es_cabecera, f_lat, f_lon, ahora
                        )
...
def vincular_o_crear_colectivo(linea, ramal, sentido, sentido_code, tiempo, es_cabecera, lat, lon, ahora):
    ...
    if bus_existente_id:
        ...
        nuevo_id = f"{linea}_{round(lat, 4)}_{round(lon, 4)}"
