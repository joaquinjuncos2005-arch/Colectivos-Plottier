...
def geocodificar_referencia_ruta(texto):
    """
    Geocodifica una intersección usando Photon.
    Se usa únicamente durante la preparación inicial de las rutas,
    nunca dentro de /api/arribos.
    """
    try:
        resp = requests.get(
...
def construir_rutas_en_segundo_plano():
    """
    Se ejecuta aparte del request web para que Render/Gunicorn
    pueda levantar la aplicación sin esperar la geocodificación.
    """
    global rutas_mapa_cache
    ...
    try:
        ...
        for linea, referencias in RUTAS.items():
            ...
            for referencia in referencias:
                punto = geocodificar_referencia_ruta(referencia)
            ...
            # Si OSRM falla, mantenemos la unión de referencias como
            # respaldo. La API nunca queda bloqueada por esto.
            if not geometria:
                geometria = puntos
        ...
        with rutas_mapa_lock:
            rutas_mapa_estado["estado"] = "error"
            rutas_mapa_estado["mensaje"] = "Error calculando recorridos"
